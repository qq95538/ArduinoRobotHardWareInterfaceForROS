...
// accuracy and such
extra=0.04; // for differencing
$fs=.02; // circle/face accuracy
$fn=60; // circle default facets
...
sun_bore_r=6.2/2; // Nema 23 D shaft dia
...
sun_teeth=24; // needs to be divisable by both <num_planets> and 2.
ring_teeth=96; // needs to be divisable by both <num_planets> and 2.
...
module view_assembly(explode=0) {
	...
	echo(str("Output Pitch: " , output_pitch*25.4));
...
module idler() {
	rotate([0,0,360/idler_teeth/1]) gear(number_of_teeth=idler_teeth, diametral_pitch=output_pitch, hub_diameter=output_pitch*sun_teeth, hub_thickness=ring_gear_h, bore_diameter=idler_cutout_r*2, rim_thickness=ring_gear_h, rim_width=0, gear_thickness=ring_gear_h,clearance=clearance, twist=-output_twist/idler_teeth,backlash=backlash, pressure_angle=pressure_angle);
}
module sun() {
	rotate([0,0,360/sun_teeth/1]) gear(number_of_teeth=sun_teeth, diametral_pitch=input_pitch, hub_diameter=input_pitch*sun_teeth, hub_thickness=ring_gear_h, bore_diameter=sun_bore_r*2, rim_thickness=ring_gear_h, rim_width=0, gear_thickness=ring_gear_h,clearance=clearance, twist=-input_twist/sun_teeth,backlash=backlash, pressure_angle=pressure_angle);
		// D shaft flat
